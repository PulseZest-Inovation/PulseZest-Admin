...
import { Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, Button, Grid } from '@mui/material';
...
export default function AppDevelopmentDetail() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const docRef = doc(db, 'appDevelopment', id);
        ...
        console.error('Error fetching app development details:', error);
      }
    };
  ...
  // Get keys of appDetails and sort them alphabetically
  const sortedKeys = Object.keys(appDetails).sort((a, b) => a.localeCompare(b));
  ...
  const handleSaveField = async (key) => {
    try {
      ...
      // Update appDetails immediately after successful save
  ...
  const handleAddField = async () => {
    try {
      ...
      setNewFieldKey('');
      setNewFieldValue('');
      // Update appDetails immediately after adding new field
      setAppDetails((prevDetails) => ({
        ...prevDetails,
        [newFieldKey]: newFieldValue,
      }));
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
      <Typography variant="h4" gutterBottom align="center">App Development Details</Typography>
